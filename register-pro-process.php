...
if (empty($registrantEmail) OR empty($registrantPassword) OR empty($registrantDateOfBirth) OR empty($registrantGender)) {	
	...
	if (filter_var($registrantEmail, FILTER_VALIDATE_EMAIL)) {
		...
		$select = mysqli_query($conn, "SELECT * FROM trainers WHERE email = '".$registrantEmail."'");
		$select2 = mysqli_query($conn, "SELECT * FROM trainees WHERE email = '".$registrantEmail."'");
if(mysqli_num_rows($select)) {	
	...
	echo ("<script LANGUAGE='JavaScript'>
    window.alert('This email is used for the account of a trainee! Please use a different email!');
    window.history.back();
    </script>");
} else {
	...
	$sql = "INSERT INTO trainers (id, firstname, lastname, email, password, date_of_birth, gender)
