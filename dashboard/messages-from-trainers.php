...
$userEmail = $_SESSION['userEmail'];  
 ...
 $sql2 = "select * from account_connections where trainee = '$userEmail' and verified = 'By Both'";  
        $result2 = mysqli_query($con, $sql2);  
        $count2 = mysqli_num_rows($result2);


 $sql3 = "select * from trainers_messages where trainee = '$userEmail' ORDER BY reg_date DESC";  
...
<h1 id="pageTitle">Trainers' Messages</h1>
	<div class="row">
		<div class="column2">
<h2 style="margin-top: 0px;">Messages Exchanged</h2>
			<p style="text-align: center;"><u>Here you can see the messages exchanged with trainers.</u></p>
	...
	for ($x = 1; $x <= $noOfPages; $x++) {
			...
			$sqlLoop = "select * from trainers_messages where trainee = '$userEmail' ORDER BY reg_date DESC LIMIT 10 OFFSET $x2";  
...
echo "<p style='padding: 5px; border: 1px solid #fff;'>There are no messages exchanged with trainers.</p>";	
}
				?>
...
<button id="prevTableButton">Prev</button>&nbsp;&nbsp;&nbsp;<button id="nextTableButton">Next</button>
		...
		<div class="column2">
		<h2 style="margin-top: 0px;">Connected Trainers' Accounts</h2>
					<p style="text-align: center;"><u>Here you can see the emails of the trainers with whom you can exchange messages.</u><br>To create a new connection with a trainer, use the relevant field in the "My Account" page.</p>
...
echo "<table>";
	echo "<tr><th><strong>Trainer's Email</strong></th></tr>";
...
echo "<p style='padding: 5px; border: 1px solid #fff;'>There are no verified connections with accounts of trainers.</p>";	
}
				?>
	...
	<hr>
		<div class="row" style="flex-direction: column;">
			...
			<h2 style="margin-top: 0px;">Send new message</h2>
					<p><u>Here you can send new messages to connected trainers' accounts.</u></p>
  ...
  padding: 20px;
  align-items: center;
  justify-content: space-between;" id="formaNewMessageToTrainer" name="formaNewMessageToTrainer" action="send-message-to-trainer-process.php"  method="POST">
					 <label for="trainersEmail">Email of Trainer</label><br>
					 <input style="padding: 10px;
  width: 100%;
  max-width: 400px; font-size: 15px;" id='trainersEmail' name='trainersEmail' type='email' placeholder='Email'><br>
					 <label for="messageToTrainer">Message to Trainer</label><br>
					 <textarea style="padding: 10px;
									  max-width: 100%; width: 400px; min-height: 200px; font-size: 15px;" id='messageToTrainer' name='messageToTrainer' placeholder='Message'></textarea><br><br>
	...
	for (let i = 0; i < deleteMessages.length; i++) {
		deleteMessages[i].addEventListener("click", function() {
		 ...
		 $.ajax({
  method: "POST",
  url: "/dashboard/delete-message-process.php",
  data: { rowID: rowID },
 success: function(data){ 	
	 if (data) {
    window.alert('The message has been removed!');
     window.location.href='/dashboard/messages-from-trainers.php';
	 } else {
		 window.alert('Error!');
  window.location.href='/dashboard/messages-from-trainers.php';
	 }
