 ...
 const dOmos_x = results.poseLandmarks[12].x; 
  ...
  const dKarp_z = results.poseLandmarks[16].z;
	...
	if (exerActive) {
		...
		if (miti_y > 1 || miti_y < 0 || dGona_y > 1 || dGona_y < 0 || aGona_y > 1 || aGona_y < 0 || dOmos_x > 1 || dOmos_x < 0 || aOmos_x > 1 || aOmos_x < 0 || dAgon_x > 1 || dAgon_x < 0 || aAgon_x > 1 || aAgon_x < 0 || dKarp_x > 1 || dKarp_x < 0 || aKarp_x > 1 || aKarp_x < 0) {
		...
		document.getElementById("areaForMessages").innerHTML = "";	
			if (chosenHand == "left") {
				...
				if (positionOfHand == "down") {
					...
					if (aAgon_y < aKarp_y && dAgon_y < dKarp_y) {
					positionOfHand = "down";
						 drawLandmarks(canvasCtx, results.poseLandmarks, {color: '#0000FF', lineWidth: 2});
				} else if (aAgon_y > aKarp_y && dAgon_y > dKarp_y) {
				if (positionOfHand == "down") {
					countRepeats++;
					if (chosenGoal == "reps") {
					document.getElementById("currentRepeat").innerHTML = (chosenReps - countRepeats) + " left";
		...
		countTime++;
				 ...
				 if (countTime < 10) {
				  ...
				  document.getElementById("currentTime").innerHTML = "1:00:00";
					 ...
					 exerActive = false;
			localStorage.setItem("bicepCurlsRepeats", countRepeats);
			 localStorage.setItem("bicepCurlsTime", 3600);
					 localStorage.setItem("bicepCurlsWeight", chosenWeight);
					  localStorage.setItem("bicepCurlsHand", chosenHand);
							  		        Swal.fire({
...
window.location.href = "/dashboard/exercises/hand-exercises/index.php";
				...
				countTime--;
				  ...
				  document.getElementById("currentTime").innerHTML = "00:00 left";
					 ...
					 exerActive = false;
			localStorage.setItem("bicepCurlsRepeats", countRepeats);
			 localStorage.setItem("bicepCurlsTime", ((chosenMinutes * 60) + chosenSeconds));
					 localStorage.setItem("bicepCurlsWeight", chosenWeight);
					  localStorage.setItem("bicepCurlsHand", chosenHand);
							  		        Swal.fire({
...
window.location.href = "/dashboard/exercises/hand-exercises/index.php";
			...
			function finishExer() {
					 ...
					 exerActive = false;
			localStorage.setItem("bicepCurlsRepeats", countRepeats);
			 localStorage.setItem("bicepCurlsTime", countTime);
					 localStorage.setItem("bicepCurlsWeight", chosenWeight);
					  localStorage.setItem("bicepCurlsHand", chosenHand);
							  		        Swal.fire({
...
<option value="time">Time</option>
  <option value="reps">Repetitions</option>
	...
	<div id="currentExerMessage">Exercise: Bicep Curls (<div id="currentHand"></div>)</div>
