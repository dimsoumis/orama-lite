  ...
  const dKarp_z = results.poseLandmarks[16].z;
	...
	if (exerActive) {
		...
		document.getElementById("areaForMessages").innerHTML = "<span style='color: #f00;'>Important Message:</span><br>Please place yourself properly in the frame. Your whole body should be visible by the camera!";	
			positionOfHand = "out";
		} else 	if (miti_y > dKarp_y || miti_y > aKarp_y) {
	document.getElementById("areaForMessages").style.display = "block";
		document.getElementById("areaForMessages").innerHTML = "<span style='color: #f00;'>Important Message:</span><br>Do not raise your hands too high!";	
			positionOfHand = "out";
		} else {
		document.getElementById("areaForMessages").style.display = "none";
		document.getElementById("areaForMessages").innerHTML = "";	
			if (chosenHand == "left") {
				if (aOmos_y < aAgon_y && aAgon_y < aKarp_y && Math.abs(aAgon_x - aOmos_x) < 0.075 && Math.abs(aKarp_x - aOmos_x) < 0.075) {
					positionOfHand = "down";
					  drawLandmarks(canvasCtx, results.poseLandmarks, {color: '#0000FF', lineWidth: 2});
				} else if (Math.abs(aOmos_y - aAgon_y) < 0.075 && Math.abs(aAgon_y - aKarp_y) < 0.075 && Math.abs(aKarp_x - aOmos_x) > 0.15) {
				...
				if (dOmos_y < dAgon_y && dAgon_y < dKarp_y && Math.abs(dAgon_x - dOmos_x) < 0.075 && Math.abs(dKarp_x - dOmos_x) < 0.075) {
					positionOfHand = "down";
						 drawLandmarks(canvasCtx, results.poseLandmarks, {color: '#0000FF', lineWidth: 2});
				} else if (Math.abs(dOmos_y - dAgon_y) < 0.075 && Math.abs(dAgon_y - dKarp_y) < 0.075 && Math.abs(dKarp_x - dOmos_x) > 0.15) {
				if (positionOfHand == "down") {
					countRepeats++;
					if (chosenGoal == "reps") {
					document.getElementById("currentRepeat").innerHTML = (chosenReps - countRepeats) + " left";
					...
					if (aOmos_y < aAgon_y && aAgon_y < aKarp_y && Math.abs(aAgon_x - aOmos_x) < 0.075 && Math.abs(aKarp_x - aOmos_x) < 0.075 && dOmos_y < dAgon_y && dAgon_y < dKarp_y && Math.abs(dAgon_x - dOmos_x) < 0.075 && Math.abs(dKarp_x - dOmos_x) < 0.075) {
					positionOfHand = "down";
						 drawLandmarks(canvasCtx, results.poseLandmarks, {color: '#0000FF', lineWidth: 2});
				} else if (Math.abs(aOmos_y - aAgon_y) < 0.075 && Math.abs(aAgon_y - aKarp_y) < 0.075 && Math.abs(aKarp_x - aOmos_x) > 0.15 && Math.abs(dOmos_y - dAgon_y) < 0.075 && Math.abs(dAgon_y - dKarp_y) < 0.075 && Math.abs(dKarp_x - dOmos_x) > 0.15) {
				if (positionOfHand == "down") {
					countRepeats++;
					if (chosenGoal == "reps") {
					document.getElementById("currentRepeat").innerHTML = (chosenReps - countRepeats) + " left";
		...
		countTime++;
				 ...
				 if (countTime < 10) {
				  ...
				  document.getElementById("currentTime").innerHTML = "1:00:00";
					 ...
					 exerActive = false;
			localStorage.setItem("lateralRaisesRepeats", countRepeats);
			 localStorage.setItem("lateralRaisesTime", 3600);
					 localStorage.setItem("lateralRaisesWeight", chosenWeight);
					  localStorage.setItem("lateralRaisesHand", chosenHand);
							  		        Swal.fire({
...
window.location.href = "/dashboard/exercises/hand-exercises/index.php";
				...
				countTime--;
				  ...
				  document.getElementById("currentTime").innerHTML = "00:00 left";
					 ...
					 exerActive = false;
			localStorage.setItem("lateralRaisesRepeats", countRepeats);
			 localStorage.setItem("lateralRaisesTime", ((chosenMinutes * 60) + chosenSeconds));
					 localStorage.setItem("lateralRaisesWeight", chosenWeight);
					  localStorage.setItem("lateralRaisesHand", chosenHand);
							  		        Swal.fire({
...
window.location.href = "/dashboard/exercises/hand-exercises/index.php";
			...
			function finishExer() {
					 ...
					 exerActive = false;
			localStorage.setItem("lateralRaisesRepeats", countRepeats);
			 localStorage.setItem("lateralRaisesTime", countTime);
					 localStorage.setItem("lateralRaisesWeight", chosenWeight);
					  localStorage.setItem("lateralRaisesHand", chosenHand);
							  		        Swal.fire({
...
<option value="time">Time</option>
  <option value="reps">Repetitions</option>
	...
	<div id="currentExerMessage">Exercise: Lateral Raises (<div id="currentHand"></div>)</div>
